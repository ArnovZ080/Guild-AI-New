...
export function MagicTextReveal({
  ...
  const createParticles = useCallback((ctx, canvas, text, textX, textY, font, color, transformedDensity) => {
    const particles = [];
      ...
      for (let x = 0; x < canvas.width; x += sampleRate) {
        ...
        if (alpha > 0) {
          ...
          particles.push({
            ...
            targetOpacity: originalAlpha * (0.6 + Math.random() * 0.4),
  ...
  const updateParticles = useCallback((particles, deltaTime, isHovered, showText, setShowText, spread, speed) => {
    ...
    particles.forEach(particle => {
      if (isHovered) {
        ...
        if (Math.abs(opacityDiff) < 0.01) {
          particle.targetOpacity = Math.random() < 0.3
            ? particle.originalAlpha * 0.5
            : particle.originalAlpha * (0.8 + Math.random() * 0.6);
  ...
  const renderParticles = useCallback((ctx, particles, globalDpr) => {
    ctx.save();
  ...
  const renderCanvas = useCallback(() => {
    ...
    const particles = createParticles(ctx, canvas, text, canvas.width / 2, canvas.height / 2, font, color, transformedDensity);
