        ...
        <div className="p-6 max-w-7xl mx-auto space-y-8">
            <header className="flex items-center justify-between mb-8">
                <div>
                    ...
                    <p className="text-zinc-500 mt-1">Strategic objectives and key results tracker.</p>
                </div>
                <button className="bg-[#1a6fff] text-white px-5 py-2.5 rounded-xl font-bold text-sm shadow-md hover:bg-[#4d8fff] transition-all flex items-center gap-2">
                    <Plus size={18} strokeWidth={1.5} /> New Goal
                </button>
            </header>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {goals.map(goal => (
                    ...
                    >
                        <div className="flex justify-between items-start mb-6">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <p className="text-xs font-bold text-zinc-600 uppercase tracking-wider">{goal.metric}</p>
                                </div>
                            </div>
                            <div className="flex items-center gap-2">
                                ...
                                <button className="text-zinc-600 hover:text-zinc-500"><MoreVertical size={16} strokeWidth={1.5} /></button>
                            </div>
                        </div>
                        ...
                        <div className="space-y-3">
                            <h4 className="text-xs font-bold text-zinc-600 uppercase mb-2">Key Milestones</h4>
                            {goal.milestones.map(ms => (
                                <div key={ms.id} className="flex items-center gap-3 p-2 hover:bg-white/[0.03] rounded-lg transition-colors cursor-pointer group">
                                    <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center ${ms.completed ? 'bg-[#1a6fff] border-indigo-600' : 'border-slate-300'}`}>
                                        {ms.completed && <CheckCircle2 size={12} className="text-white" strokeWidth={1.5} />}
                                    </div>
                                    <span className={`text-sm font-medium ${ms.completed ? 'text-zinc-600 line-through' : 'text-zinc-400'}`}>
                                        {ms.title}
                                    </span>
                                </div>
                            ))}
                        ...
                        <div className="mt-6 pt-4 border-t border-slate-100 flex items-center justify-between">
                            <div className="flex items-center gap-2 text-xs font-bold text-zinc-500">
                                <Calendar size={14} strokeWidth={1.5} /> Deadline: {goal.deadline}
                            </div>
                            <button className="text-[#1a6fff] text-sm font-bold hover:underline flex items-center gap-1">
                ...
                <motion.div
                    ...
                    className="bg-zinc-900/60 rounded-2xl p-6 shadow-xl text-white relative overflow-hidden"
                >
